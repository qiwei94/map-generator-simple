...
parser = argparse.ArgumentParser(
    description="使用 osmium CLI 管线生成上海 3MF 模型"
)
parser.add_argument(
    '--elevation-file',
    type=str,
...
# PBF 文件
PBF_FILE = os.path.join(_project_root, 'pbf_cache', 'shanghai-latest.osm.pbf')
if not os.path.exists(PBF_FILE):
    print(f"ERROR: PBF file not found: {PBF_FILE}")
    sys.exit(1)

# 上海 25km 区域
LAT1, LON1 = 31.13, 121.35
LAT2, LON2 = 31.35, 121.63
CITY_NAME = "shanghai_cli"
OUTPUT_DIR = "output/shanghai_cli"

# Sub-mesh on/off (一次性调参开关，改完跑即可)
ENABLE_VEGETATION = False
ENABLE_BLOCK_BASE = True
# True = block_base + BO 合并为统一城市底层（2-layer 模式：city_block + landmarks）
# False = 三层分离（block_base E6 + buildings E1 + landmarks E5）
MERGE_BLOCK_LAYERS = True
...
if cli_args.sample_only:
    ...
    if not osmium:
        print("  ERROR: osmium not found"); sys.exit(1)
    ...
    n_buildings_est = -1
    try:
        subprocess.run([osmium, 'extract', '-b', f'{LON1},{LAT1},{LON2},{LAT2}', '-s', 'smart', PBF_FILE, '-o', _pbf_extract, '--overwrite'], capture_output=True, timeout=120, check=True, creationflags=_flags)
        subprocess.run([osmium, 'tags-filter', _pbf_extract, 'nwr/building', '-o', _pbf_buildings, '--overwrite'], capture_output=True, timeout=60, check=True, creationflags=_flags)
        fsize = os.path.getsize(_pbf_buildings)
        n_buildings_est = max(0, fsize // 300)
        print(f"  建筑 PBF: {fsize/1024:.1f} KB, 估算建筑数: ~{n_buildings_est:,}")
    except Exception as e:
        print(f"  采样失败: {e}")
    finally:
        shutil.rmtree(_tmp_dir, ignore_errors=True)
    area_km2_est = (LAT2-LAT1)*111*(LON2-LON1)*111*np.cos(np.radians((LAT1+LAT2)/2))
    density = n_buildings_est/area_km2_est if area_km2_est>0 and n_buildings_est>0 else 0
    print(f"  面积: {area_km2_est:.1f} km², 密度: ~{density:.0f} /km²")
    if n_buildings_est > 500000: print(f"  ⚠️ 超高密度！建议缩小 bbox (≤ 0.1°)")
    elif n_buildings_est > 100000: print(f"  ℹ️ 高密度，MERGE+landmarks 已启用")
    else: print(f"  ✅ 密度正常")
    print(f"  耗时: {time.time()-t_s:.1f}s"); sys.exit(0)
...
# MERGE 模式下只需提取地标建筑（building_landmarks 过滤器），
# 避免加载全量建筑数据（巴黎 1.9M → ~几百个），节省 20+ 分钟
_bldg_tag = 'building_landmarks' if MERGE_BLOCK_LAYERS else 'building'
