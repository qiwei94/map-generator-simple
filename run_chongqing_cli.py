...
parser = argparse.ArgumentParser(
    description="使用 osmium CLI 管线生成重庆 3MF 模型"
)
parser.add_argument(
    '--elevation-file',
    type=str,
...
# PBF 文件
PBF_FILE = os.path.join(_project_root, 'pbf_cache', 'chongqing-latest.osm.pbf')
if not os.path.exists(PBF_FILE):
    print(f"ERROR: PBF file not found: {PBF_FILE}")
    sys.exit(1)

# 重庆 25km 区域
LAT1, LON1 = 29.45, 106.45
LAT2, LON2 = 29.67, 106.7
CITY_NAME = "chongqing_cli"
OUTPUT_DIR = "output/chongqing_cli"
...
if cli_args.sample_only:
    ...
    n_buildings_est = -1
    try:
        subprocess.run([osmium, 'extract', '-b', f'{LON1},{LAT1},{LON2},{LAT2}',
                        '-s', 'smart', PBF_FILE, '-o', _pbf_extract, '--overwrite'],
                       capture_output=True, timeout=120, check=True, creationflags=_flags)
        subprocess.run([osmium, 'tags-filter', _pbf_extract, 'nwr/building',
    ...
    if n_buildings_est > 500000:
        print(f"  ⚠️  超高密度！建议缩小 bbox 到核心区 (经纬度跨度 ≤ 0.1°)")
    elif n_buildings_est > 100000:
        print(f"  ℹ️  高密度，MERGE + building_landmarks 已自动启用")
    else:
        print(f"  ✅ 密度正常，可安全运行")
