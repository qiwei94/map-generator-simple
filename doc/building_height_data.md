...
`identity-anchor-background-v2` 在压缩真实高度之前先决定建筑的视觉角色。仅有普通
商业名称不足以获得真实 Z；必须具备知识库、历史文化、用途类型等身份信号，或成为
可靠的全城高度上尾锚点：

- `identity_exact`：有身份标签（Wikidata/Wikipedia、历史文化、宗教、交通/公共
  枢纽等）且有可信高度；使用来源高度、类别强调和完整层高量化；
- `visual_anchor_exact`：没有身份标签，但属于全城可靠高度上尾的少量建筑；使用
  OSM 明确高度、Wikidata 或 Overture 高度，成为天际线锚点；
- `identity_stylized`：有身份意义但缺少可信高度；使用确定性的地标风格高度，
  不把面积估算冒充真实楼高；
- `background_stylized`：普通无名建筑，即使有 `building:levels` 或因面积被个体
  保留，也只进入安静、统一的低对比高度带。

普通背景建筑保留原始平面轮廓来维持城市密度，但不再获得地标式横向膨胀或周边
“广场”退避。这样准确数据服务于城市身份与视觉中心，普通楼群保持风格一致，且
所有决定均由标签、来源、城市分位数和打印层高确定，不由 LLM 直接控制 Z。
...
可打印地标的模型高度范围，并新增 `height_role_policy_version` 和
`printable_height_role_counts`。由此可以回答某个 3MF 到底用了哪批高度证据、多少
建筑采用真实高度、多少建筑被风格化，以及为什么形成当前 Z，而不是仅证明数据库
文件存在。
